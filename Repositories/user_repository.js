...
user_repository.create = (user) => db('user').insert(user).returning('*');
...
user_repository.find_by_identification = (identification) =>
  db('user').select('*').where('identification', identification).first();
